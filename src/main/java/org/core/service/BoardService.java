...
public class BoardService {
  ...
  public BoardService(int teamCount, int pieceCount, BoardType boardType) {
    this.gamePiecesManager = new GamePiecesManager(new GamePieceInitializer(), teamCount,
        pieceCount);
  ...
  public List<GamePieces> findAllPlacedPiecesByTeam(int team) {
    return gamePiecesManager.findAllPiecesByTeam(team).stream()
        .filter(piece -> !piece.getPlace().equals("start")).toList();
  }
